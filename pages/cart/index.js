...
export default function Cart() {
    ...
    for (const productId of cartProducts) {
        const price = products.find(p => p._id === productId)?.price || 0;
        const pricee = Number(price)
        subTotal += Math.floor(pricee + total / 1000);
    }
                                            ...
                                            <div key={product._id} className="mt-8">
                                                <ul className="space-y-4">
                                                    <li className="flex items-center gap-4 justify-between">
                                                        ...
                                                        <div>
                                                            <h3 className="text-xl text-text max-w-md">{product.title}</h3>

                                                            <dl className="mt-0.5 space-y-px text-lg text-text">
                                                                <p>₹ {product.price}</p>
                                                            </dl>
                                                        </div>
                                        ...
                                        <div className="mt-8 flex justify-end border-t border-gray-100 pt-8">
                                            <div className=" max-w-md space-y-4">
                                                ...
                                                <div className="flex justify-end">
                                                    <Link
                                                        class="group flex items-center justify-between gap-4 rounded-lg border border-current px-4 py-2 text-red-600 transition-colors hover:bg-red-600 focus:outline-none focus:ring active:bg-red-500"
                                                        href="/products"
                                                    >
                                                        ...
                                                        <span
                                                            class="shrink-0 rounded-full border border-red-600 bg-white p-2 group-active:border-red-500"
                                                        >
                                                            <svg
                                ...
                                <div class="space-y-5">
                                    <div class="grid grid-cols-12 gap-5">
                                        ...
                                        <div class="col-span-12 text-center w-full">
                                            <button onClick={stripeCheckout} className="disabled block rounded bg-red-400 px-5 py-3 text-lg text-white transition hover:bg-red-300 w-full">
                                                Checkout
                                            </button>
                                        </div>
                                    </div>
    ...
    return <>
        <div className="grid h-screen px-4 bg-white place-content-center">
            <div className="text-center">
                ...
                <p className="mt-4 text-text text-4xl">You should sign Up to view cart Items</p>

                <button
                    onClick={() => signIn('google')}
                    className="inline-block px-5 py-3 mt-6 text-xl font-medium text-text bg-red-200 ring-red-300 ring-2 rounded hover:bg-primary focus:outline-none focus:ring"
                >
                    Login / Register
                </button>
            </div>
